...
*   Wait for the Dask `Release Publisher` workflow to complete successfully.
    ...
    versions match the tag, smoke-tests both artifacts on supported Python
...
*   Wait for the Distributed `Release Publisher` workflow to complete
    ...
    until `dask==YYYY.M.X` is available on PyPI, smoke-tests both artifacts,
    publishes them to PyPI with Trusted Publishing, and publishes the GitHub
    Release.
